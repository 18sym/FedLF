    ...
    def _weight_initialization(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # 行代码计算了卷积核的大小，并将结果存储在变量 n 中。m.kernel_size 是卷积核的大小，m.out_channels 是卷积核的输出通道数。
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                # 这行代码对当前卷积层的权重进行正态分布初始化。m.weight.data 是权重的数据，normal_ 方法用于将权重初始化为均值为0，标准差为 math.sqrt(2.0 / n) 的正态分布。
